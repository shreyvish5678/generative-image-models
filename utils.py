...
class UpsampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels=None): 
        super().__init__()
        channels = in_channels if out_channels is None else out_channels
...
def get_time_embedding(t, n_time=32):
    freqs = torch.pow(10000, -torch.arange(start=0, end=n_time // 2) / (n_time // 2)).to(t.device)
    x = t[:, None] * freqs[None, :]
