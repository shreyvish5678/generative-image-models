...
class DiffusionUNet(nn.Module):
    def __init__(self, config):
        ...
        self.time_proj = nn.Sequential(
            nn.Linear(self.n_time, self.n_time * 4), nn.SiLU(),
            nn.Linear(self.n_time * 4, self.n_time * 4)
        ).to(self.device)
        self.first_layer = nn.Sequential(   
            nn.Conv2d(self.in_channels, channels[0], kernel_size=3, padding=1),
            nn.SiLU(),
        ...
        self.final_layer = nn.Sequential(
            nn.GroupNorm(32, channels[0]), 
            nn.SiLU(),
            nn.Conv2d(channels[0], self.in_channels, kernel_size=3, padding=1)
        ).to(self.device)   
        self.encoders = nn.ModuleList()
        self.bottleneck = nn.ModuleList()
        self.decoders = nn.ModuleList()
        for i in range(len(channels) - 1):
            self.encoders.append(nn.Conv2d(channels[i], channels[i+1], kernel_size=3, stride=2, padding=1))
            for j in range(depth):
                self.encoders.append(ResidualBlock(channels[i+1], channels[i+1], self.n_time * 4))
            self.encoders.append(AttentionBlock(channels[i+1]))
        bottle_channels = [channels[-1]]
        for i in range(blocks - 1):
            bottle_channels.append(channels[-1] * 2)
        bottle_channels.append(channels[-1])
        bottle_channels.append(channels[-1])
        for i in range(len(bottle_channels) - 1):
            self.bottleneck.append(ResidualBlock(bottle_channels[i], bottle_channels[i+1], self.n_time * 4))
        channels.reverse()
        for i in range(len(channels) - 1):
            self.decoders.append(ResidualBlock(channels[i] * 2, channels[i], self.n_time * 4))
            for j in range(depth-1):
                self.decoders.append(ResidualBlock(channels[i] * 2, channels[i], self.n_time * 4))
            self.decoders.append(AttentionBlock(channels[i]))
            self.decoders.append(UpsampleBlock(channels[i], channels[i+1]))
    ...
    def forward(self, x, t):
        ...
        for layer in self.encoders:
            ...
            skip_connections.append(x) if isinstance(layer, ResidualBlock) else None
        for layer in self.bottleneck:
            x = layer(x, t)
        for layer in self.decoders:
            x = torch.cat([x, skip_connections.pop()], dim=1) if isinstance(layer, ResidualBlock) else layer(x)
            x = layer(x, t) if isinstance(layer, ResidualBlock) else x
